...
class CaffeDNNFaceDetector(ROIDetector):
    ...
    def detect(self, frame):
        ...
        # cv2.dnn.blobFromImage对图像进行预处理，1.0为图像各通道数值的缩放比例，self.blob_size表示输出图像的空间尺寸
        # self.color_mean用于各通道减去的值，用于降低光照的影响
...
class FaceMeshDetector(ROIDetector):
    ...
    _lower_face = [105, 104, 103, 332, 333, 334]
    ...
    def detect(self, frame):
        ...
        #print('wuwyuiyui', landmarks, 'hhjfgh')
        # 468个点的坐标
        # 使用了results.multi_face_landmarks[0]函数，landmarks的属性长度为468，回归预测[x, y, z]关键点的个数
...
def get_facemesh_coords2(frame):
    ...
    #             landmarks_list = [landmarks.part(i).x, landmarks.part(i).y] + landmarks_list
    ...
    landmarks_list = np.array(landmarks_list)
    return landmarks_list
# class FaceMeshDetector2(ROIDetector):
#     def __init__(self, draw_landmarks=False, refine=False, **kwargs):
#         super().__init__(**kwargs)
#         self.face_mesh = mp.solutions.face_mesh.FaceMesh(
#             max_num_faces=1,
...
#             self.draw_facemesh(frame, results.multi_face_landmarks,
#                                tesselate=True)
#         landmarks = get_facemesh_coords2(frame)
#         facerect = get_boundingbox_from_landmarks(landmarks)
#         bgmask = get_default_bgmask(frame.shape[1], frame.shape[0])
#         return RegionOfInterest.from_contour(rawimg, landmarks, facerect=facerect, bgmask=bgmask)
#
#     def draw_facemesh(self, img, multi_face_landmarks, tesselate=False,
#                       contour=False, irises=False):
#         if multi_face_landmarks is None:
